...
var Game = function Game() {
  ...
  this.newGameButtonSub.addEventListener('click', function () {
    return _this.newGame();
  });
...
Game.prototype.placeCards = function () {
  ...
  for (var i = 0; i < this.dealerCards.length; i++) {
    var cardNode = this.createCardNode();
    ...
    var node = this.dealerArea.appendChild(cardNode); //Add card to dealer area.
    this.dealerCards[i].node = node; //Used to id the card.
  }

  //Player
  for (var _i = 0; _i < this.playerCards.length; _i++) {
    var _cardNode = this.createCardNode();

    //Card image
    var _imageNode = document.createElement("IMG");
    _imageNode.src = this.playerCards[_i].path;
    _cardNode.appendChild(_imageNode);

    var _node = this.playerArea.appendChild(_cardNode);
    this.playerCards[_i].node = _node;
  }
};
...
Game.prototype.clearCards = function () {
  ...
  if (this.playerCards.length > 0) {
    for (var _i2 = 0; _i2 < this.playerCards.length; _i2++) {
      if (this.playerCards[_i2].node != null) {
        this.playerArea.removeChild(this.playerCards[_i2].node);
      }
    }
  }
};
...
Game.prototype.swapHiddenCard = function (hiddenCard) {
  for (var i = 0; i < this.dealerCards.length; i++) {
    if (this.dealerCards[i].cardtext == 'hidden') {
      //this.dealerArea.removeChild(this.dealerCards[i].node);
      var node = this.dealerCards[i].node;
