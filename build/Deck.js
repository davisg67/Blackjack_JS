...
Deck.prototype.shuffleDeck = function () {
  var shuffleCount = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 1;
  ...
  for (var x = 0; x < shuffleCount; x++) {
    ...
    for (var i = 0; i < size - 1; i++) {
      var swapIndex = Math.floor(Math.random() * size); //Get random index from 0-51.

      //Swap current card with random card.
      var tmp = this.cards[swapIndex];
